...
export async function DELETE(
  ...
  try {
    ...
    if (!user || !["COMPANY_ADMIN", "BUILDING_OWNER"].includes(user.role) || user.companyId !== companyId) {
      return NextResponse.json({ ok: false, message: "Only this company's user can remove this parking allocation." }, { status: 403 });
    }

    await prisma.$transaction(async (tx) => {
      await lockRfid(tx);
      const vehicle = await tx.vehicle.findFirst({
        where: { id: vehicleId, employeeId, companyId },
      ...
      await tx.rfidEnrollment.updateMany({
        where: { vehicleId, status: { in: ["WAITING", "CAPTURED"] } },

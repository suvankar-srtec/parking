...
function canEditEmployee(
  ...
  employee: {
    companyId: string;
    company: { buildingId: string; building: { superAdminId: string | null } };
  },
) {
  ...
  if (user.role === "BUILDING_ADMIN") {
    return user.buildingId === employee.company.buildingId;
  }
  if (user.role === "SUPER_ADMIN") {
    return !user.createdBySuperAdminId || employee.company.building.superAdminId === user.id;
  }
  return false;
}
...
export async function PATCH(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (parkingLimit < employee._count.vehicles) {
        throw new EmployeeUpdateError(
          `Parking limit cannot be lower than ${employee._count.vehicles} because this person already has ${employee._count.vehicles} registered vehicle${employee._count.vehicles === 1 ? "" : "s"}.`,
        );
      }

      const otherTotals = await tx.employee.aggregate({
        where: { companyId, id: { not: employeeId } },
        _sum: { parkingLimit: true },
      });
      ...
      if (parkingLimit > availableForThisPerson) {
        throw new EmployeeUpdateError(
          `This parking limit is too high. A maximum of ${availableForThisPerson} space${availableForThisPerson === 1 ? "" : "s"} can be assigned to this person.`,
        );
      }
    ...
    return NextResponse.json({
      ok: true,
      message: `${result.category === "OWNER" ? "Company owner" : "Employee"} updated successfully.`,
      employee: {
        id: result.id,
        name: result.name,

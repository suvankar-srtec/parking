...
export default function LoginPage() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    void execute(async () => {
      const result = await requestJson<{ ok: boolean; redirectTo: string }>("/api/login", "POST", {
        userId: data.get("userId"), username: data.get("username"), password: data.get("password"),
      });
  ...
  return <main className="login-page"><section className="login-card">
    ...
    <div className="login-heading"><span>WELCOME BACK</span><h1>Sign in</h1><p>Enter your User ID, username and password.</p></div>
    <form className="login-form" noValidate aria-busy={pending} onSubmit={submit}>
      ...
      <label>Username<input name="username" placeholder="Username" required autoComplete="off" disabled={pending} /></label>

...
export default async function RegisterCardsPage() {
  ...
  return <main className="dashboard-page">
          ...
          <table className="rfid-register-table">
            ...
            <tbody>
              {vehicles.length ? vehicles.map((vehicle) => <tr key={vehicle.id}>
                ...
                <td>
                  <span className={`rfid-type-badge ${vehicle.employee.category === "OWNER" ? "owner" : "employee"}`}>
                    {vehicle.employee.category === "OWNER" ? "Company Owner" : "Employee"}
                  </span>
                </td>
                <td><span className="rfid-vehicle-number">{vehicle.plateNumber}</span></td>
                <td>
                  {vehicle.rfidCardNo
                    ? <span className="rfid-card-badge registered">{vehicle.rfidCardNo}</span>
                    : <span className="rfid-card-badge unregistered">Not registered</span>}
                </td>
              </tr>) : <tr>
                <td colSpan={6} className="rfid-empty-row">No vehicles are available for card registration.</td>
              </tr>}
            </tbody>
          </table>
        </div>
    ...
    <style>{`
      ...
      .rfid-register-table thead th:first-child{border-top-left-radius:8px}
      .rfid-register-table thead th:last-child{border-top-right-radius:8px}
